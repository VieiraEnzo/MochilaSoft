...
class Model {
    ...
    public:
        ...
        ~Model();
        std::pair<Solution, int> Build_Model(ProblemInstance* _p, vector<int> best_solution);
        std::pair<Solution, double> Build_Model_with_LB(ProblemInstance* _p, vector<int> best_solution);
        std::pair<Solution, int> Build_Model_with_Patterns(ProblemInstance* _p, int num_patterns, const std::vector<std::vector<int>>& pattern_matrix, vector<int> best_solution, int best_value);
        status getStatus();
